...
Usage:
  python 梯度提升决策树/gbdt_whitebox.py --train 梯度提升决策树/data/train.csv
  python 梯度提升决策树/gbdt_whitebox.py --train 梯度提升决策树/data/train.csv --test 梯度提升决策树/data/test.csv --out 梯度提升决策树/output/submission.csv
"""
...
def main():
    ...
    ap.add_argument("--test", default=None,
                    help="Path to test CSV (default: data/test.csv if it exists)")
    ap.add_argument("--target", default="label")
    ap.add_argument("--out", default=None,
                    help="Submission CSV path (default: output/submission.csv)")
    # GBDT hyper-params
    ...
    ap.add_argument("--export_artifacts", action="store_true",
                    help="Save feature importance/model/tree dumps inside artifact_dir.")
    ap.add_argument("--artifact_dir", default=None,
                    help="Directory for optional artifacts (default: output/).")
    args = ap.parse_args()
    ...
    default_test = DEFAULT_DATA_DIR / "test.csv"
    if args.test:
    ...
    out_path = Path(args.out) if args.out else DEFAULT_OUTPUT_DIR / "submission.csv"
    artifact_dir = Path(args.artifact_dir) if args.artifact_dir else DEFAULT_OUTPUT_DIR
    ...
    if args.export_artifacts:
        ...
        fi.sort_values("importance_gain", ascending=False).to_csv(artifact_dir / "feature_importance_gbdt.csv", index=False)
        with open(artifact_dir / "gbdt_trees.json","w",encoding="utf-8") as f:
            json.dump(gbdt.dump(), f, ensure_ascii=False, indent=2)
        with open(artifact_dir / "gbdt_model.pkl","wb") as f:
            pickle.dump({
                "encoder": enc,
                "model": gbdt,
        ...
        with open(artifact_dir / "gbdt_config.json","w",encoding="utf-8") as f:
            json.dump({
        ...
        print("[INFO] Skipped artifact export; only submission.csv will be generated.")

    # Predict test
    if test_path and test_path.exists():
        tdf = pd.read_csv(test_path).copy()
        if "id" not in tdf.columns:
            raise ValueError("test.csv is missing the 'id' column")
        ...
        print("[INFO] --test not provided; skipped submission export.")
